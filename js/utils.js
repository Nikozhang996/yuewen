...
var $ = (function () {
  ...
  function siblings(element) {
    return prevAll(element).concat(nextAll(element));
  }
  ...
  function ajax(options) {
    ...
    if (_default.type === 'get') {
      ...
      // 判断是否需要问号传参
      if (typeof _default.data !== 'string' && _default.data != null) {

        var queryArr = [];
        for (var key in _default.data) {
          if (_default.data.hasOwnProperty(key)) {
            queryArr.push(encodeURIComponent(key) + '=' + encodeURIComponent(_default.data[key]));
          }
        }
        _default.url += queryArr.join('&');
      }
    }
      ...
      if (/^2\d{2}$/.test(xhr.status)) {
        ...
        if (xhr.readyState === 4) {
          ...
          if (typeof _default.success === 'function') {
            _default.success.call(xhr, result);
          }
        } else {
    ...
    if (typeof _default.complete === 'function') {
      _default.complete.call(xhr);
    }
    xhr.send(null);
  }
